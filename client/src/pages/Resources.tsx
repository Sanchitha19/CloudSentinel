...
export const Resources: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] text-gray-100 p-8 font-sans">
      ...
      {loading ? (
        <div className="text-blue-500 text-center py-20">Loading resources...</div>
      ) : activeTab === 'inventory' ? (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {displayRecommendations.length === 0 ? (
              ...
              return (
                <div key={rec.id} className={`bg-[#1e212b] p-6 rounded-xl border transition-all duration-300 relative overflow-hidden group 
                  ${applied ? 'border-green-500/30 opacity-70 bg-green-900/5' : dismissed ? 'border-gray-800 opacity-50' : 'border-gray-700 hover:border-gray-500 hover:shadow-2xl hover:-translate-y-1'}`}
                >
                  ...
                  <div className="flex gap-2 mb-4 pr-10">
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded 
                      ${rec.priority === 'high' ? 'bg-red-500/20 text-red-500' : rec.priority === 'medium' ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'}`}>
